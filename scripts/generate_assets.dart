...
void main() {
  ...
  _generateSubClass(className: 'AppImages', basePath: 'assets/images', folders: ['png', 'svg', 'png/mascot', 'png/backgrounds', 'svg/placeholders'], extensions: ['.png', '.svg'], buffer: bufferImages, outputFile: '$outputRoot/images/app_images.dart');

  _generateSubClass(className: 'AppAnimations', basePath: 'assets/animations', folders: ['gif', 'lottie'], extensions: ['.gif', '.json'], buffer: bufferAnimations, outputFile: '$outputRoot/animations/app_animations.dart');

  _generateSubClass(className: 'AppSounds', basePath: 'assets/sounds', folders: [], extensions: ['.wav', '.mp3'], buffer: bufferSounds, outputFile: '$outputRoot/sounds/app_sounds.dart');
  ...
  bufferMainAssets.writeln("  static const _basePath = 'packages/pln_click_design_system';\n");

  _writeMainMapping(bufferMainAssets, 'AppIcons', 'icon', outputRoot);
  _writeMainMapping(bufferMainAssets, 'AppImages', 'image', outputRoot);
  _writeMainAssetsMapping(bufferMainAssets, 'AppAnimations', 'animation', outputRoot);
  _writeMainMapping(bufferMainAssets, 'AppSounds', 'sound', outputRoot);

  bufferMainAssets.writeln('}');

  File('$outputRoot/app_assets.dart').writeAsStringSync(bufferMainAssets.toString());

  // ignore: avoid_print
  print('✅ [PLN Click] All Assets Generated in lib/src/assets Successfully!');
}

void _generateSubClass({required String className, required String basePath, required List<String> folders, required List<String> extensions, required StringBuffer buffer, required String outputFile}) {
  buffer.writeln("// GENERATED CODE - DO NOT MODIFY BY HAND");
  buffer.writeln('abstract class $className {');
  buffer.writeln('  $className._();');

  final List<String> targetPaths = [basePath, ...folders.map((f) => '$basePath/$f')];
  ...
  for (var path in targetPaths) {
    ...
    final files = dir.listSync().whereType<File>().where((f) {
      final extMatch = extensions.any((ext) => f.path.toLowerCase().endsWith(ext));
    ...
    for (var file in files) {
      ...
      if (RegExp(r'^(icon|ic|img|animation)[-_][a-zA-Z0-9]+').hasMatch(rawName)) {
        cleanName = rawName;
      } else {
        cleanName = rawName.replaceAll('icon-', '').replaceAll('ic-', '').replaceAll('ic_', '').replaceAll('img-', '').replaceAll('img_', '').replaceAll('animation-', '');
      }
      ...
      if (currentFolder == 'outlined' && !cleanName.toLowerCase().contains('outlined')) {
        suffix = 'Outlined';
      } else if (currentFolder == 'colored' && !cleanName.toLowerCase().contains('colored')) {
        suffix = 'Colored';
      }
...
void _writeMainMapping(StringBuffer buffer, String subClassName, String prefix, String outputRoot) {
  final file = File('$outputRoot/${_getFolderByClass(subClassName)}/${_getFileNameByClass(subClassName)}');
  ...
  for (var line in lines) {
    if (line.contains('static const String')) {
      ...
      buffer.writeln("  static const $finalVarName = '\$_basePath/\${$subClassName.$varNameFromSubClass}';");
    }
  }
  buffer.writeln('');
}

void _writeMainAssetsMapping(StringBuffer buffer, String subClassName, String prefix, String outputRoot) {
  _writeMainMapping(buffer, subClassName, prefix, outputRoot);
}

String _getFolderByClass(String cls) => cls.replaceFirst('App', '').toLowerCase();
